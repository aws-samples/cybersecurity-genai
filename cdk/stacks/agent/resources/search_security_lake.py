...
class SearchSecurityLake(Construct):
    def __init__(self, scope: Construct, construct_id: str, aoss_endpoint: str, aoss_collection_id: str, aoss_collection_map: Dict, **kwargs) -> None:
        ...
        ssm_parameter = aws_ssm.StringParameter(
            self,
            id=SearchSecurityLakeProps.PARAMETER_ID,
            parameter_name=SearchSecurityLakeProps.PARAMETER_NAME,
        ...
        createstream_statement = aws_iam.PolicyStatement(
            ...
            resources=[
                f"arn:aws:logs:{Stack.of(self).region}:{Stack.of(self).account}:log-group:/aws/lambda/{SearchSecurityLakeProps.LAMBDA_NAME}:*"
            ],
        )
        ...
        lambda_iam_policy = aws_iam.ManagedPolicy(
            self,
            id=SearchSecurityLakeProps.LAMBDA_IAM_POLICY_ID,
            managed_policy_name=SearchSecurityLakeProps.LAMBDA_IAM_POLICY_NAME,
            statements=[
                createlog_statement,
        ...
        lambda_iam_role = aws_iam.Role(
            self,
            id=SearchSecurityLakeProps.LAMBDA_IAM_ROLE_ID,
            role_name=SearchSecurityLakeProps.LAMBDA_IAM_ROLE_NAME,
        ...
        lambda_layer_opensearchpy = aws_lambda.LayerVersion(
            self,
            id=SearchSecurityLakeProps.LAMBDA_LAYER_OPENSEARCHPY_ID,
            layer_version_name=SearchSecurityLakeProps.LAMBDA_LAYER_OPENSEARCHPY_NAME,
            code=aws_lambda.Code.from_asset('stacks/agent/lambda_layers/opensearch_py/layer.zip'),
            compatible_runtimes=[
        ...
        search_security_lake_lambda = aws_lambda.Function(
            self,
            id=SearchSecurityLakeProps.LAMBDA_ID,
            function_name=SearchSecurityLakeProps.LAMBDA_NAME,
            description=SearchSecurityLakeProps.LAMBDA_DESCRIPTION,
            code=aws_lambda.Code.from_asset('stacks/agent/lambda_functions/search_security_lake/'),
